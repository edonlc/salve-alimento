<?php
$tituloPagina = 'Editar doação — Salve Alimento';
include __DIR__ . '/../layout/cabecalho.php';
?>

<div class="flex-entre">
    <h1 class="pagina-titulo" style="margin-bottom:0">Editar doação</h1>
    <a href="/painel" class="btn btn-secundario">← Voltar</a>
</div>

<div class="card mt-2" style="max-width:600px">
    <form method="POST" action="/doacoes/editar">
        <?= $csrfCampo ?>
        ...
        <div class="form-grupo">
            ...
            <input type="text" id="titulo" name="titulo" required maxlength="120"
                   value="<?= htmlspecialchars($doacao['titulo'] ?? '') ?>">
        </div>

        <div class="form-grupo">
            <label for="descricao">Descrição</label>
            <textarea id="descricao" name="descricao" maxlength="500"><?= htmlspecialchars($doacao['descricao'] ?? '') ?></textarea>
        </div>
        ...
        <div class="form-grupo">
            ...
            <input type="date" id="dt_limite_retirada" name="dt_limite_retirada" required
                   min="<?= date('Y-m-d') ?>"
                   value="<?= htmlspecialchars($doacao['dt_limite_retirada'] ?? '') ?>">
        </div>
        ...
        <div class="form-grupo">
            ...
            <input type="text" id="endereco_retirada" name="endereco_retirada" required maxlength="255"
                   value="<?= htmlspecialchars($doacao['endereco_retirada'] ?? '') ?>">
        </div>
